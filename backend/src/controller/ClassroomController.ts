...
export const createClassroom: RequestHandler = async (req, res) => {
	if (!req.body.name || !req.body.description) {
		return res
			.status(400)
			.json(errorMessage(2, "Name and description are required"));
	}
...
export const getAllClassrooms: RequestHandler = async (req, res) => {
	...
	const hasPermission = adminFlags.every((flag) =>
		userFlagValues.includes(flag)
	);
...
export const addStudentToClassroom: RequestHandler = async (req, res) => {
	...
	try {
		...
		return res
			.status(400)
			.json(errorMessage(11, "User is already in the classroom"));
	}

	res.status(200).send(
		successMessage(54, "User added to classroom successfully")
	);
};
...
export const removeStudentFromClassroom: RequestHandler = async (req, res) => {
	...
	try {
		...
		return res
			.status(400)
			.json(errorMessage(16, "User is not in classroom anymore"));
	}

	res.status(200).send(
		successMessage(56, "Removed Student: " + userId + " from classroom")
	);
};
...
export const getAllStudentsInClassroom: RequestHandler = async (req, res) => {
	...
	if (isNaN(Number(classroomId))) {
		return res
			.status(400)
			.json(errorMessage(3, "Classroom ID is not valid"));
	}
	...
	try {
		...
		return res
			.status(400)
			.json(errorMessage(12, "Could not retrieve students"));
	}
};
